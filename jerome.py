...
def welcome():
    ...
    if session.get('logged_in'):
        # on recupere la liste ds radios ()show_media) et des podcast (show_podcast) pour les afficher avec un bouton pour les jouer
        #radio = show_media()
	...
	if request.method == 'POST':
	    ...
	    # Fonction Ajout (si le champ hidden ajouter_radio n'est pas vide on attribue les valeurs du formulaire aux variables > arguments
	    # pour la fonction add_media()
	    # MARCHE PAS ??
	    ...
	    # Fonction modification (si le champ hidden ajoutradio n'est pas vide on attribue les valeurs du formulaire aux variables > arguments
	    # pour la fonction update_media()
	    # MARCHE PAS ??
        ...
        return render_template('welcome.html',
				...
				#music = music,
				#testcron = testcron,
				heures = recup_heure().strftime('%H'), minutes = recup_heure().strftime('%M'), secondes = recup_heure().strftime('%S'))
